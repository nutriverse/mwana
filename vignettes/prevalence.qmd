...
Under the hood, `compute_combined_prevalence()` applies the same analysis approach as in `mw_estimate_prev_wasting_wfhz` and in `compute_muac_prevalence()`. It checks the acceptability of the standard deviation of WFHZ and MFAZ and of the age ratio test. The following conditionals are checked and applied: 
