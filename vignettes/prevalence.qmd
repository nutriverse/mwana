...
To estimate the prevalence of wasting based on WFHZ we use the `mw_estimate_prev_wasting_wfhz()` function. The dataset to supply must have been wrangled by `mw_wrangle_wfhz()`. 
...
This job is assigned to `mw_estimate_prev_wasting_muac()`. Once you call the function, before starting the prevalence estimation, it first evaluates the acceptability of the MFAZ standard deviation and the age ratio test. Yes, you read well, MFAZ's standard deviation, not on the raw values MUAC.
...
Under the hood, `compute_combined_prevalence()` applies the same analysis approach as in `mw_estimate_prev_wasting_wfhz` and in `mw_estimate_prev_wasting_muac()`. It checks the acceptability of the standard deviation of WFHZ and MFAZ and of the age ratio test. The following conditionals are checked and applied: 
