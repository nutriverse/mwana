...
To estimate the prevalence of wasting based on WFHZ we use the `mw_estimate_wfhz_prevalence` function. The dataset to supply must have been wrangled by `mw_wrangle_wfhz()`. 
...
Under the hood, `compute_combined_prevalence()` applies the same analysis approach as in `mw_estimate_wfhz_prevalence` and in `compute_muac_prevalence()`. It checks the acceptability of the standard deviation of WFHZ and MFAZ and of the age ratio test. The following conditionals are checked and applied: 
