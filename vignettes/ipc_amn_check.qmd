...
Now that we are acquainted with the data set, we can proceed to execute the task. To achieve this, we simply do:
...
A table (of class `tibble`) is returned with three columns:  
  ...
  + Column `n_clusters` counts the number of unique cluster IDs in the data set.  
...
The above output is not quite useful yet as we often deal with multiple area data set. We can get a summarised table by area as follows: 
```{r}
#| label: group_by
#| echo: true
#| eval: false
