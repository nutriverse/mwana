...
`mw_plausibility_check_wfhz()` is a replica of the plausibility check in ENA for SMART software of the SMART Methodology [@smart2017]. Under the hood, it runs the same statistical tests you already know from SMART, and it applies the same rating and scoring criteria. Beware though that there are some small differences to have in mind: 
...
It is always a good practice to start off by inspecting the data set. Let's check the first 6 rows of our data set:
...
When working on a multiple area data set, for instance districts, we can check the plausibility of all districts in the data set at once by simply chaining the previous workflow with `group_by()` function from the `dplyr` package: 
