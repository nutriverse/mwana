...
As mentioned somewhere above, before we supply data object to `check_plausibility_wfhz()`, we need to wrangle it first. This task is executed by `mw_wrangle_age()` and `process_wfhz_data()`. Read more about the technical documentation by doing this `help(mw_wrangle_age)` or `help(process_wfhz_data)` in `R` console. 
