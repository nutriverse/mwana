...
As mentioned somewhere above, before we supply data object to `check_plausibility_wfhz()`, we need to wrangle it first. This task is executed by `mw_wrangle_age()` and `mw_wrangle_wfhz()`. Read more about the technical documentation by doing this `help(mw_wrangle_age)` or `help(mw_wrangle_wfhz)` in `R` console. 
...
We will assess the plausibility of the MUAC data through it's raw values when the variable age is not available in the dataset. This is a job assigned to `check_plausibility_muac()`. The workflow for this check is the shortest one.
