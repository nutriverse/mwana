...
#' @export
#'
mw_stattest_ageratio2 <- function(age_cat, .expectedP = 0.66) {
  ## Enforce the class of `age_cat` ----
  if (!is.character(age_cat)) {
    stop(
      "`age_cat` must be of class `character` not ", class(age_cat),
      ". Please try again."
    )
  }

  ## Enforce that categories in `age_cat` are as expected ----
  if (!all(levels(as.factor(age_cat)) %in% c("6-23", "24-59"))) {
    stop(
      'Unexpected categories in `age_cat`. Please consider re-coding it to "6-23"
      and "24-59". Please try again.'
    )
  }

  ## Calculate observed proportion and ratio ----
  sum_o24 <- sum(na.omit(age_cat == "24-59"))
  sum_u24 <- sum(na.omit(age_cat == "6-23"))
  total <- sum(table(na.omit(age_cat)))
  ratio <- sum_o24 / sum_u24
  prop <- sum_o24 / total
