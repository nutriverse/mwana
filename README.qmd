...
It automates plausibility checks and prevalence analyses and respective output summaries.
...
digraph mwana {
  ...
  node6 [label = 'MUAC', shape = note];
  node7 [label = 'Process age', shape = box];
  node8 [label = 'Process data', shape = box];
  ...
  no -> node10 [lable = 'NO', color = gray, arrowhead = vee];
  
  ## Workflow for Yes WFZH ----
  node3 -> node4 [color = gray, arrowhead = vee];
  node4 -> node5 [color = gray, arrowhead = vee];
  node5 -> node10 [color = gray, arrowhead = vee];
...
::: {.callout-tip}
if you are undertaking a research and you want to wrangle your data before including in your statistical models, etc, `mwana` is a great resource.
:::
...
If you were enticed to using `mwana` package and found it useful, please cite using the suggested citation provided by a call to `citation` function as follows:
